...
import com.qyk.util.PasswordEncoder;
...
public class UserLoginPresenter {
    ...
    public void login() {
        ...
        if (StringUtils.isBlank(userLoginView.getPassword()) || StringUtils.isBlank(userLoginView.getUserName())) {
            userLoginView.showFailedError("用户名，密码不能为空");
            userLoginView.hideLoading();
            return;
        }
        PasswordEncoder passwordEncoder = new PasswordEncoder("", "MD5");
        Call<ResultData<TeacherInfo>> response = teacherService.login(userLoginView.getUserName(), passwordEncoder.encode(userLoginView.getPassword()));
